...
#todo перевести на общий класс импорта чеков
class Plugin(BP.BasePlugin):
    def run(self):
        ...
        self.sqlparams = self.xml_get_all_params_from_file(file_sql_text.find('params'), as_dic=True)
        ...
        rbssumsale = self.ExecuteSQL('select * from RBQ_Q_GET_SALE_BY_CHECK(?,?)',
                                     sqlparams = [docsale, None],
                                     fetch='many',
                                     ExtVer=True)
        if rbssumsale['status'] == krconst.kr_sql_error:
            self.LogFile(krconst.m_e_cash_get_sales_from_rbs + krconst.kr_term_double_enter)
            return False
        else:
            ...
            for itm in rbssumsale['datalist']:
                ...
                if mssqlsumsale:
                    ...
                    if self.docidsale or self.docidret:
                        ...
                        ''' Если чеков нет, нужно все равно провести выравнивание '''
                        resstatus = self.ExecuteSQL('select * from RBS_Q_SALE_STATUS(?,?)',
                                                    sqlparams = [None, None],
                                                    fetch='one',
                                                    ExtVer=True)
    ...
    def get_sum_from_mssql(self, docsale, externalobjid):
        ...
        if execsql['status'] == krconst.kr_sql_error:
            self.LogFile(krconst.m_e_cash_get_sales_from_cash % 'MSSQL' + krconst.kr_term_double_enter)
            return None
        else:
            if execsql['datalist']:
    ...
    def get_check_head_from_mssql(self, docsale, externalobjid, objid, doctype):
        ...
        if execsql['status'] == krconst.kr_sql_error:
            self.LogFile(krconst.m_e_cash_get_head_from_cash % docsale + krconst.kr_term_double_enter)
            return None
        else:
            for check in execsql['datalist']:
                ''' сохраним шапку чека в БД '''
                savedoc = self.ExecuteSQL('select * from RBS_Q_SALE_DOCUMENT_SAVE(?,?,?,?,?,?,?,?)',
                                          sqlparams = [objid, check._DOCIDCASH, check._DOCTYPE, check._EXTERNALOBJCHECKID, TimeStampToDateTime(check._DOCDATE), check._NUMBER, check._DOCSUMM, None],
                                          fetch='one',
                                          ExtVer=True,
                                          auto_commit=False
                                          )

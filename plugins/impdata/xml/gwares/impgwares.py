...
class Gwares(object):
        ...
        if self.__obj is not None:
            ...
            if characteristics is not None:
                for itm in characteristics:
                    ...
                    delete_marker = str_to_bool_int(self._xml_get_value(itm, 'deletemarker'))
    ...
    def save(self):
        ...
        sql_text = 'execute procedure RBS_Q_GWARES_INSSEL(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'

        characteristic = []
        if len(self._characteristic) == 0:
            characteristic.append({'name': None,
                                   'delete_marker': None})
        else:
            characteristic = self._characteristic

        for secondary_name in characteristic:
    ...
    def barcode(self, barcodes, create_ttx, unit=None):
        """
            ШК
        """
        ...
        if barcodes is not None:
            for barcode in barcodes:
                wu_delete_marker = self._xml_get_value(barcode, 'deletemarker', flag='N')
                wu_delete_marker = str_to_bool_int(wu_delete_marker)
                if wu_delete_marker == 0 or wu_delete_marker is None:
                    ...
                    wares_barcode = BarcodeToDic(wares_barcode, self._xml_get_value(barcode, 'value'), unit, factor,
                                                 uweight, ulength, uheight, uwidth, characteristic)
            if wares_barcode:
                ...
                if len(self._characteristic) == 0:
                    ...
                    for itm in wares_barcode:
                        sql_params = [self._code, itm['unit'], itm['barcode'], itm['factor'],
                    ...
                    for itm in wares_barcode:
                        if itm['characteristic'] is not None:
                            sql_params = [self._code, itm['unit'], itm['barcode'], itm['factor'],
                                          itm['uweight'], itm['ulength'], itm['uheight'], itm['uwidth'],
                    ...
                    for char_name in self._characteristic:
                        for unit_name in unit_list:
                            for itm in wares_barcode:
                                if itm['characteristic'] is None and itm['unit'] == unit_name:
                                    sql_params = [self._code, itm['unit'], itm['barcode'], itm['factor'],
                                                  itm['uweight'], itm['ulength'], itm['uheight'], itm['uwidth'],
    ...
    def _wares_unit(self, wares_units, type_wares_unit):
        ...
        for obj in wares_units:
            if self._xml_get_value(obj, 'type') == type_wares_unit:
                unit = self._xml_get_value(obj, 'unit')
                ...
                sql_params = [self._code, unit, factor, type_wares_unit, ulength, uwidth, uheight,
                              uweight, unetweight, self._parent_class.coef_ttx, None, self._external_id]

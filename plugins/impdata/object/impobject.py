...
class Obj():
    ...
    def save(self):
        ...
        sql_params = [self.code, self.name, self.namefull,
                      ...
                      # закоментировал было для ктц
                      #, self.order_max_time,
                      #self.delivery_time
                      ]
        if self.limit_use_by_date is not None:
            sql_text += ',?'
            sql_params += [self.limit_use_by_date]
        sql_text += ')'
        ...
        if res['status'] == k.kr_sql_error:
            ...
            if self.format:
                ...
                sql_params = [self.format, self.objid, self.shop_code]
                res = self.parent_class.ExecuteSQL(sql_text,
                                                   sqlparams = sql_params,
                                                   fetch='none',
                                                   ExtVer=True)

...
class Man(imp_object.Obj):
    ...
    def __init__(self, parent_class, flag_work, obj=None):
        ...
        if obj is not None:
            ...
            self.email = self.xml_get_value(obj, 'email', flag='N')
            ...
            # временный костыль для АЗ
            self.external_id = self.xml_get_value(obj, 'id1c', flag='N')
    ...
    def set_email(self):
        ...
        if self.objid:
            sql_text = 'execute procedure Q_MAN_SET_EMAIL(?, ?)'
            sql_params = [self.objid, self.email]
            ...
            res = self.parent_class.ExecuteSQL(sql_text,
                                               ...
                                               ExtVer=True)
            if res['status'] == k.kr_sql_error:
                message = k.m_e_i_set_email % self.code
                self.parent_class.log_file(message,
                                           terms=0,
                                           save_log_db=True)
                self.result_class = k.plugin_error

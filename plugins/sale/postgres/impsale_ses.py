...
"""
    ivaikin 30.12.2016
    version 0.0.3.1
    Модуль импорта чеков из postgres SES кристалл
"""
...
class ImportSES():
    ...
    def __init__(self, parent_class):
        ...
        self.sale_rbs = s.BaseImpSale(self.parent_class, 'SES10')

        '''
            получим из параметров задания code POSTGRES БД
            если коннекта к БД нет выходим
        '''
        self.db_cash_connect = self.parent_class.PostgreSQLConnect(self.sale_rbs.qt_params['code_db_SES10'])
    ...
    def save_head(self, header, obj_id):
        ...
        for check in header:
            ...
            doc = s.BaseImpSaleDoc(self.parent_class, 'SES')
            doc.obj_id = obj_id
            doc.doc_id_cash = check.DOCIDCASH
            doc.doc_type_code = check.DOCTYPE
            doc.external_obj_id = str(check.EXTERNALOBJCHECKID)
            doc.doc_date = check.DOCDATE
            doc.doc_number = str(check.NUMBER)
            doc.doc_sum = check.DOCSUMM
    ...
    def save_detail(self, detail, doc_sales_id):
        ...
        for wares in detail:
            cargo = s.BaseImpSaleCargo(self.parent_class, 'SES')
            cargo.doc_sales_id = doc_sales_id
            cargo.wares_code = wares.WARESCODE
            cargo.amount = wares.AMOUNT
            cargo.price = wares.PRICE
            cargo.summa = wares.SUMMA

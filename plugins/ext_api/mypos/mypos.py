...
class Api(object):
    ...
    def put_gwares(self, deviceid, data):
        ...
        self.__barcode_uses = []
        for i in wares:
            gwares.append('%s;%s;%s;;%.2f;;;%s;;%.1f;;;;;;%s;1;' % (i['code'], '', i['name'], i['price'], i['is_weight_wares'], i['discount'], i['parentid']))
            # шк
            for barcode in snull(i['barcodes']).split(','):
                if len(barcode) > 0:
                    barcode = barcode.strip()
                    if self.__tmp_check_barcode(barcode):
                        gwares.append('#%s;%s;%s;%s;%.2f;;;;;1;' % (i['code'], barcode, i['name'], i['name'], i['price']))
            ...
            if i['PROP_PDFSTAMP']:
                if i['PROP_PDFSTAMP'] == 1:
                    pdf_stamp = '0'
                else:
                    pdf_stamp = '1'
                gwares.append('<goods_attr id="%s" attr_id="24">%s</goods_attr>' % (i['code'], pdf_stamp))
            if i['PRODUCERINN']:
            ...
            if i['alcoholbyvolume']:
                gwares.append('<goods_attr id="%s" attr_id="27">%s</goods_attr>' % (i['code'], i['alcoholbyvolume']))
            pid = i['parentid']
            while pid:
                g = groups[pid]
    ...
    def __post(self, path, data):
        ...
        # todo Щеглов: нужно обработка ошибок в плагин, что бы понятна была ошибка
    ...
    def __tmp_check_barcode(self, barcode):
        """
        Проверка был ли ранее данный ШК
        @param barcode: 
        @return: barcode
        """
        ...
        if len(self.__barcode_uses) > 0 :
            for itm in self.__barcode_uses:
                if barcode == itm:
                    return None

            self.__barcode_uses.append(barcode)
            return barcode
        else:
            self.__barcode_uses.append(barcode)
        return barcode
